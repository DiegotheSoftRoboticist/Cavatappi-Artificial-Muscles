    ...
    if length(Lable)>=n
        ...
        for i=1:n
            if ~sum(strcmpi({'' },Lable(i)))
                text(LableX(i), LableY(i),cell2mat(Lable(i)), 'FontSize',8,'HorizontalAlignment','center','Rotation',0)
            end
        end
    end
else
    ...
    if length(LineColor)>=m
        for i=1:m
            if sum(strcmpi({'y' 'm' 'c' 'r' 'g' 'b' 'w' 'k' },LineColor(i)))
                set(A(i),'Color',cell2mat(LineColor(i)),'LineWidth',0.4)
            end
        end
    end
else
